  ...
  methods: {
    ...
    updateCountry() {
      ...
      for (let i = 0; i < countries.length; i++) {
        if (Number(countries[i].countryCode) === Number(this.selectedCountryToEdit.countryCode)) {
          isCountryCodeReiterative = true;
        }
        if (countries[i].iso3.toLowerCase() === this.selectedCountryToEdit.iso3.toLowerCase()) {
          isIso3Reiterative = true;
        }
      }
      ...
      if (isCountryCodeReiterative || isIso3Reiterative) {
        ...
        axios.put(vars.api_base3 + '/Country/UpdateCountry', this.selectedCountryToEdit).then(response => {
          if (response.data.isSuccess) {
            this.$q.notify({
              type: 'positive',
              message: 'اطلاعات کشور با موفقیت بروزرسانی شد.'
            })
            this.getCountries()
