        ...
        <div class="row">
          <div class="col-12 col-md-6 q-mt-md">
            <div class="input">
              ...
              <q-input v-model="selectedQuestionToEdit.title" outlined dense />
            </div>
          </div>
          <div class="col-12 col-md-6 q-mt-md">
            ...
            <div class="create-question-wrapper">
              <div class="row">
                ...
                <div class="col-12 q-mt-md">
                  <div class="input">
                    <span class="label"
                      >فایل تصویری (ترتیب انتخاب عکس مهم میباشد)</span
                    >
                    <q-file
                ...
                <div class="col-12">
                  <div
                    class="row"
                    v-for="(image, index) in numOfUploadedImages"
                    :key="index"
                  >
                    <div class="col-12 col-md-6 q-mt-md">
                      <div class="input">
  ...
  <q-dialog v-model="updateImagesDialog">
    <q-card>
      ...
      <q-card-section>
        <div
          class="image q-mb-sm"
          v-for="(image, index) in selectedQuestionToEdit.videoQuestion"
          :key="index"
        >
          <div class="row">
            <div class="col-3">
            ...
            <div class="col-9">
              <div class="flex column">
                ...
                />
                <span class="q-pt-sm block" v-if="changedImages[index]">{{
                  changedImages[index].name
                }}</span>
              </div>
            </div>
          </div>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      examNameOptions: [{ value: 1, label: "wisc-iv" }],
      questions: [],
      columns: [
        ...
        { name: "title", align: "center", label: "عنوان سوال", field: "title" },
        {
          ...
          field: "categoryQuestionLabel",
        },
        {
          name: "typeQuestion",
          align: "center",
          label: "نوع سوال",
          field: "typeQuestionLabel",
        },
        {
          name: "answersCount",
          align: "center",
  ...
  methods: {
    setQuestion() {
      ...
      let fd = new FormData();
      if (
        !this.createQuestionData.title ||
        !this.createQuestionData.typeQuestion ||
        !this.createQuestionData.index ||
        !this.createQuestionData.categoryQuestion ||
        !this.createQuestionData.tryNumber ||
        !this.createQuestionData.time ||
        !this.createQuestionData.examName
      ) {
        this.$q.notify({
          type: "negative",
        ...
        if (!isReiterativeIndex) {
          ...
          fd.append(
            "typeQuestion",
            Number(this.createQuestionData.typeQuestion.value)
          );
          fd.append("index", this.createQuestionData.index);
          fd.append(
            "categoryQuestion",
            this.createQuestionData.categoryQuestion.value
          );
          ...
          if (Object.keys(this.createQuestionData.image).length !== 0) {
            for (let i = 0; i < this.numOfUploadedImages; i++) {
              if (
                this.createQuestionData.image[i].type === "image/png" ||
                this.createQuestionData.image[i].type === "image/jpeg"
              ) {
                videoQuestion[i] = {
          ...
          axios
            .post(vars.api_base + "/Questions/CreateQuestion", data)
            .then((response) => {
              ...
              if (response.data.isSuccess) {
                ...
                this.$q.notify({
                  type: "negative",
                  message:
                    response.data.exceptions[0].exception.persianDescription,
                });
              }
            })
            .catch((error) => {
    ...
    onDocumentPicked(file) {
      this.createQuestionData.soundQuestion = URL.createObjectURL(
        file.target.files[0]
      );
    },
    onFailedDocumentPicked(file) {
      this.createQuestionData.soundFailed = URL.createObjectURL(
        file.target.files[0]
      );
    },
    getAllQuestion(reqProps) {
      ...
      axios
        .post(vars.api_base + "/Answer/GetAnswers", {
          searchQuery: null,
          index: null,
          take: null,
          skip: null,
      ...
      axios
        .post(vars.api_base + "/Questions/GetQuestions", {
          searchQuery: null,
          index: null,
        ...
        .then((response) => {
          ...
          for (let i = 0; i < this.questions.length; i++) {
            switch (this.questions[i].typeQuestion) {
              case 1:
                this.questions[i] = {
                  typeQuestionLabel: "قرار دادنی",
                  ...this.questions[i],
                };
                break;
              case 2:
                this.questions[i] = {
                  typeQuestionLabel: "صدا",
                  ...this.questions[i],
                };
                break;
              case 3:
                this.questions[i] = {
                  typeQuestionLabel: "انتخاب کردنی",
                  ...this.questions[i],
                };
                break;
              case 4:
                this.questions[i] = {
                  typeQuestionLabel: "خط زدنی",
                  ...this.questions[i],
                };
                break;
              default:
            ...
            switch (this.questions[i].categoryQuestion) {
              case 1:
                this.questions[i] = {
                  categoryQuestionLabel: "طراحی با مکعب\u200Cها",
                  ...this.questions[i],
                };
                break;
              case 2:
                this.questions[i] = {
                  categoryQuestionLabel: "شباهت\u200Cها",
                  ...this.questions[i],
                };
                break;
              case 3:
                this.questions[i] = {
                  categoryQuestionLabel: "فراخوانی ارقام",
                  ...this.questions[i],
                };
                break;
              case 4:
                this.questions[i] = {
                  categoryQuestionLabel: "مفاهیم تصویری",
                  ...this.questions[i],
                };
                break;
              case 5:
                this.questions[i] = {
                  categoryQuestionLabel: "رمز نویسی",
                  ...this.questions[i],
                };
                break;
              case 6:
                this.questions[i] = {
                  categoryQuestionLabel: "واژگان",
                  ...this.questions[i],
                };
                break;
              case 7:
                this.questions[i] = {
                  categoryQuestionLabel: "توالی حرف و عدد",
                  ...this.questions[i],
                };
                break;
              case 8:
                this.questions[i] = {
                  categoryQuestionLabel: "استدلال تصویری",
                  ...this.questions[i],
                };
                break;
              case 9:
                this.questions[i] = {
                  categoryQuestionLabel: "درک مطلب",
                  ...this.questions[i],
                };
                break;
              case 10:
                this.questions[i] = {
                  categoryQuestionLabel: "نمادیابی",
                  ...this.questions[i],
                };
                break;
              case 11:
                this.questions[i] = {
                  categoryQuestionLabel: "تکمیل تصاویر",
                  ...this.questions[i],
                };
                break;
              case 12:
                this.questions[i] = {
                  categoryQuestionLabel: "خط زنی",
                  ...this.questions[i],
                };
                break;
              case 13:
                this.questions[i] = {
                  categoryQuestionLabel: "اطلاعات عمومی",
                  ...this.questions[i],
                };
                break;
              case 14:
                this.questions[i] = {
                  categoryQuestionLabel: "حساب",
                  ...this.questions[i],
                };
                break;
              case 15:
                this.questions[i] = {
                  categoryQuestionLabel: "استدلال کلامی",
                  ...this.questions[i],
                };
                break;
              default:
    ...
    updateQuestion() {
      ...
      if (
        !this.selectedQuestionToEdit.title ||
        !this.selectedQuestionToEdit.typeQuestionLabel ||
        !this.selectedQuestionToEdit.index ||
        !this.selectedQuestionToEdit.categoryQuestionLabel ||
        !this.selectedQuestionToEdit.tryNumber ||
        ...
        if (!isReiterativeIndex) {
          if (Object.keys(this.chosenSound).length !== 0) {
            if (this.chosenSound.type === "audio/mpeg") {
              fd.append(
                "mediaQuestion",
                JSON.stringify(this.selectedQuestionToEdit.mediaQuestion)
              );
              fd.append("files", this.chosenSound);
            } else {
              this.$q.notify({
          ...
          if (Object.keys(this.chosenFailedSound).length !== 0) {
            if (this.chosenFailedSound.type === "audio/mpeg") {
              fd.append(
                "mediaFailed",
                JSON.stringify(this.selectedQuestionToEdit.mediaFailed)
              );
              fd.append("files", this.chosenFailedSound);
            } else {
              this.$q.notify({
          ...
          if (
            this.selectedQuestionToEdit.index !== this.selectedQuestionIndex
          ) {
          ...
          if (
            this.selectedQuestionToEdit.typeQuestionLabel.constructor === Object
          ) {
            fd.append(
              "typeQuestion",
              Number(this.selectedQuestionToEdit.typeQuestionLabel.value)
            );
            newTypeQuestion = Number(
              this.selectedQuestionToEdit.typeQuestionLabel.value
            );
          } else {
            fd.append(
              "typeQuestion",
              Number(this.selectedQuestionToEdit.typeQuestion)
            );
            newTypeQuestion = Number(this.selectedQuestionToEdit.typeQuestion);
          }
          if (
            this.selectedQuestionToEdit.categoryQuestionLabel.constructor ===
            Object
          ) {
            fd.append(
              "categoryQuestion",
              this.selectedQuestionToEdit.categoryQuestionLabel.value
            );
            newCategoryQuestion = Number(
              this.selectedQuestionToEdit.categoryQuestionLabel.value
            );
          } else {
            fd.append(
              "categoryQuestion",
              Number(this.selectedQuestionToEdit.categoryQuestion)
            );
            newCategoryQuestion = Number(
              this.selectedQuestionToEdit.categoryQuestion
            );
          }
          let data = {
          ...
          axios
            .put(vars.api_base + "/Questions/UpdateQuestion", data)
            .then((response) => {
              // console.log(response)
              if (response.data.isSuccess) {
                this.getAllQuestion();
    ...
    deleteQuestion(question) {
      ...
      axios
        .delete(vars.api_base + `/Questions/DeleteQuestion?id=${question.id}`)
        .then((response) => {
          if (response.data.isSuccess) {
            this.$q.notify({
              type: "info",
    ...
    addNewImage(index) {
      ...
      this.selectedQuestionToEdit.videoQuestion[index].url =
        this.changedImage.__key;
    },
    ...
    updateQuestionImages() {
      for (
        let i = 0;
        i < this.selectedQuestionToEdit.videoQuestion.length;
        i++
      ) {
        this.selectedQuestionToEdit.videoQuestion[i] = {
          ...
          locationX: Number(
            this.selectedQuestionToEdit.videoQuestion[i].locationX
          ),
          locationY: Number(
            this.selectedQuestionToEdit.videoQuestion[i].locationY
          ),
        };
      }
      axios
        .put(
          vars.api_base + "/Questions/UpdateQuestion",
          this.selectedQuestionToEdit
        )
        .then((response) => {
          if (response.data.isSuccess) {
